...
3. **Phase-table reconciliation.** For each phase not `done` in PHASES.md, check whether
   the files its spec names were modified by commits the workflow didn't make (commits
   since the phase's last notes entry). If yes, flag it: the spec may describe a world
   that no longer exists. Recommend per phase: re-run `/expand-phase` (status back to
   `pending` — cheap, specs are disposable by design) or, if untouched, leave as is.
