...
1. Phase `$1` exists in `docs/phases/PHASES.md` with status `pending`. (`expanded`/`in-progress` → it's already past this step; `done` → nothing to do.)
2. Every phase in its `depends` column has status `done`. If not, name the unmet dependencies — expanding on top of unfinished dependencies produces a spec built on guesses, which is the exact failure P4 exists to prevent.
...
1. **Absorb what changed.** Read the `notes.md` of every dependency phase. List every
   ...
   (see the repair protocol in `docs/constraints.md`); tell the operator which phases in
   PHASES.md need re-cutting and why.
